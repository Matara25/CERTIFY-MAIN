...
async function fixAdmin() {
    try {
        ...
        const adminAddress = accounts[0]; // Use first account
        console.log('Using Ganache account:', adminAddress);

        // First, delete any existing admin users to avoid unique constraint conflicts
        console.log('\nCleaning up existing admin users...');
        await User.deleteMany({ role: 'admin' });
        console.log('Deleted existing admin users');

        // Create new admin user with Ganache account
        ...
        const admin = new User({
            ...
            blockchainAddress: adminAddress // Use first Ganache account
        });
